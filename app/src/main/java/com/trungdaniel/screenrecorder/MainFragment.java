...
import com.trungdaniel.screenrecorder.service.RecorderService;
...
import static com.trungdaniel.screenrecorder.ScreenRecorder.AUDIO_AAC;
import static com.trungdaniel.screenrecorder.ScreenRecorder.VIDEO_AVC;
...
public class MainFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mMediaProjectionManager = (MediaProjectionManager) getActivity().getApplicationContext().getSystemService(MEDIA_PROJECTION_SERVICE);
        mNotifications = new Notifications(getActivity().getApplicationContext());
    ...
    private void init(View view) {
        ...
        mButton = view.findViewById(R.id.record_button);
        mButton.setOnClickListener(this::onButtonClick);

        mVideoCodec = view.findViewById(R.id.video_codec);
        mVieoResolution = view.findViewById(R.id.resolution);
        mVideoFramerate = view.findViewById(R.id.framerate);
        mIFrameInterval = view.findViewById(R.id.iframe_interval);
        mVideoBitrate = view.findViewById(R.id.video_bitrate);
        mOrientation = view.findViewById(R.id.orientation);

        mAudioCodec = view.findViewById(R.id.audio_codec);
        mVideoProfileLevel = view.findViewById(R.id.avc_profile);
        mAudioBitrate = view.findViewById(R.id.audio_bitrate);
        mAudioSampleRate = view.findViewById(R.id.sample_rate);
        mAudioProfile = view.findViewById(R.id.aac_profile);
        mAudioChannelCount = view.findViewById(R.id.audio_channel_count);

        mAudioToggle = view.findViewById(R.id.with_audio);
        mAudioToggle.setOnCheckedChangeListener((buttonView, isChecked) ->
                view.findViewById(R.id.audio_format_chooser)
        ...
        mVideoCodec.setOnItemSelectedListener((view1, position) -> onVideoCodecSelected(view1.getSelectedItem()));
        mAudioCodec.setOnItemSelectedListener((view1, position) -> onAudioCodecSelected(view1.getSelectedItem()));
        mVieoResolution.setOnItemSelectedListener((view1, position) -> {
            onResolutionChanged(position, view1.getSelectedItem());
        });
        mVideoFramerate.setOnItemSelectedListener((view1, position) -> {
            onFramerateChanged(position, view1.getSelectedItem());
        });
        mVideoBitrate.setOnItemSelectedListener((view1, position) -> {
            onBitrateChanged(position, view1.getSelectedItem());
        });
        mOrientation.setOnItemSelectedListener((view1, position) -> {
            onOrientationChanged(position, view1.getSelectedItem());
        });
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        getActivity().findViewById(R.id.container).setPadding(horizontal, vertical, horizontal, vertical);
    }
    ...
    private void resetSampleRateAdapter(MediaCodecInfo.CodecCapabilities capabilities) {
        ...
        if (old == null || !(old instanceof ArrayAdapter)) {
            ArrayAdapter<Integer> adapter = new ArrayAdapter<>(getActivity(), android.R.layout.simple_spinner_item, new ArrayList<>());
            adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    private SpinnerAdapter createCodecsAdapter(MediaCodecInfo[] codecInfos) {
        ArrayAdapter<String> adapter = new ArrayAdapter<>(getActivity(), android.R.layout.simple_spinner_item, codecInfoNames(codecInfos));
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        return adapter;
    }
    ...
    private void xinQuyenGhiDe() {
        if (Build.VERSION.SDK_INT >= 23) {
            if (!Settings.canDrawOverlays(getContext())) {
                Intent intent = new Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION,
                        Uri.parse("package:$packageName"));
                startActivityForResult(intent, 1234);
                Toast.makeText(getContext(), "Bạn cần cấp một số quyền cho ứng dụng", Toast.LENGTH_SHORT)
                        .show();
            }
        }
    }
